...
<div class="note-box">In OTM, access control for a user is defined through a <strong>User Role</strong> — a container that groups two distinct types of restrictions that must be configured together:
...
VPD answers <em>what data can this user see?</em> ACL answers <em>what can this user do?</em> Both are attached to the User Role and must be planned together — a gap in either can expose data or functionality that should be restricted.</div>
