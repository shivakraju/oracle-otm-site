...
Log in to the OTM application, navigate to the URL above (replacing `otm-server` with your server hostname), and click the **List** button to see all current properties.

**To modify the properties file:**

On the app and web server, switch to the base OTM directory (`$OTM_HOME`) and locate the file at:

```
$OTM_HOME/glog/config
```

Take a backup of the current file, apply your changes, and repeat on both the app and web servers. Restart the application for new settings to take effect.

**Sample properties file with key settings:**
...
# list of available app servers to poll for network topology - used only by web server. one per line.
...
#--------------------------------------------------------------------
# Default E1 Integration Properties
#--------------------------------------------------------------------
